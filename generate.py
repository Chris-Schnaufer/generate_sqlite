...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--BETYDB_URL', help="the URL to the  BETYdb server to query")
    parser.add_argument('--BETYDB_KEY', help="the BETYdb key to use when accessing the BETYdb server")
...
def get_cultivars(betydb_url: str, betydb_key: str, cultivar_json_file: str = None) -> list:
...
def match_cultivar_to_site(site_id: int, all_cultivars: list) -> Optional[tuple]:
    ...
    logging.debug("Finding cultivar for site ID: %s", str(site_id))
    ...
    logging.debug("Didn't find a cultivar for site")
    return None
...
def get_save_experiments(dates: tuple, db_conn: sqlite3.Connection, betydb_url: str, betydb_key: str,
                         experiment_json_file: str = None, cultivar_json_file: str = None) -> Optional[tuple]:
    ...
    # Get the cultivars
    all_cultivars = get_cultivars(betydb_url, betydb_key, cultivar_json_file)

    # Create the experiments table
    exp_cursor = db_conn.cursor()
    exp_cursor.execute('''CREATE TABLE experimental_info
                          (id INTEGER, season TEXT, cultivar_id INTEGER, plot_name TEXT, plot_id INTEGER,
    ...
    for found_exp in found_experiments:
        for one_site in found_exp['sites']:
            ...
            # Find out cultivar
            cultivar_match = match_cultivar_to_site(cur_site['id'], all_cultivars)
            ...
            for one_cultivar in cultivars_matched:
                if one_cultivar['id'] == cultivar_match['id']:
                    already_added = True
                    break
            ...
            exp_cursor.execute("INSERT INTO experimental_info VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               cur_site[id], found_exp['name'], cultivar_match['id'], site_name, one_site['id'],
    ...
    logging.debug("Write %s experiments records", str(total_records))
...
def save_cultivars(cultivars: list, db_conn: sqlite3.Connection) -> None:
    ...
    for one_cultivar in cultivars:
        cult_cursor.execute("INSERT INTO cultivars VALUES(?, ?)", one_cultivar['id'], one_cultivar['name'])
...
def globus_get_files_details(client: globus_sdk.TransferClient, endpoint_id: str, files_path: str) -> Optional[list]:
    ...
    globus_save_path = os.path.join('/', GLOBUS_LOCAL_START_PATH, os.path.basename(json_file))
    transfer_setup = globus_sdk.TransferData(client, endpoint_id, GLOBUS_LOCAL_ENDPOINT_ID, label="Get metadata",
                                             sync_level="checksum")
    transfer_setup.add_item(json_file, globus_save_path)
    transfer_request = client.submit_transfer(transfer_setup)
    task_result = client.task_wait(transfer_request['task_id'], timeout=600, polling_interval=10)
    ...
    for idx in file_details:
        file_details[idx] = {**more_details, **(file_details[idx])}
...
def get_save_files(remote_endpoint: str, sensor_paths: tuple, date_experiment_ids: dict, db_conn: sqlite3.Connection) -> None:
    """Fetches file information associated with the sensors and dates and updates the database
    Arguments:
...
def generate() -> None:
    ...
    try:
        ...
        get_save_files(args.globus_endpoint, sensor_paths, date_experiment_ids, sql_db)
    finally:
        if sql_db:
            sql_db.close()
        del sql_db
